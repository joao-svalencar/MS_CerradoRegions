...
db <- read.csv("baseunique_alt.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
db_full <- read.csv("BD_endemics.csv", stringsAsFactors=FALSE, fileEncoding="latin1")
be <- read.csv("spp_hier_2_015.csv")
...
head(db_be_full)
db_be_full <- db_be_full[,c(1:13, 17:19)] #remove duplicidade em classe, familia e genero
...
table(db_be_full$Taxa) #numero de registros por classe
#Amphibians         Aves      Lizards      Mammals       Snakes Squamata (A) 
#     3084          6457         2664          840        10834           74 
...
unique(db_be$Species[db_be$BEs=="X"]) #verifica as spp que compoem o BE "x"
